...
asos_std <- read_csv(file.path(RAW_DIR, "ASOS_raw.csv"), 
...
saveRDS(asos_std, file.path(PROC_DIR, "asos_daily.rds"))

# [B] AWS 표준화 (ID 앞에 'W' 추가)
raw_bytes <- readBin(file.path(RAW_DIR, "AWS_raw.csv"), "raw", file.info(file.path(RAW_DIR, "AWS_raw.csv"))$size)
...
saveRDS(aws_std, file.path(PROC_DIR, "aws_daily.rds"))

# [C] MTW 표준화 (ID 앞에 'M' 추가)
mtw_std <- readRDS(file.path(RAW_DIR, "raw_mtw_API_2017_2025.rds")) %>%
  transmute(
...
saveRDS(mtw_std, file.path(PROC_DIR, "mtw_raw_standard.rds"))